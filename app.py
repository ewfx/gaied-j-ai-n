...
def load_classifier():
    try:
        return pipeline("zero-shot-classification", model="facebook/bart-base", device=-1)
...
def classify_request_type(text):
    try:
        ...
        logger.error(f"Error in classify_request_type: {str(e)}")
        raise
...
def classify_sub_request_type(text, request_type):
    ...
    if sub_types:
        try:
            ...
            logger.error(f"Error in classify_sub_request_type: {str(e)}")
            raise
    return None
...
def parse_email(email_content):
    try:
        ...
        logger.error(f"Error in parse_email: {str(e)}")
        raise
